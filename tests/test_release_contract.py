...
class ReleaseContractTests(unittest.TestCase):
    ...
    def test_production_excludes_experimental_stack(self) -> None:
        ...
        derivative_install = text.index('git tag "v0.25.1+r0b0tlab.w4a4.1"')
        installed_audit_context = text.index("WORKDIR /", derivative_install)
        ...
        runtime_copy = runtime_stage.index("COPY --from=builder /opt/vllm/ /opt/vllm/")
        revision_arg = runtime_stage.index("ARG IMAGE_REVISION=uncommitted")
        image_label = runtime_stage.index("LABEL org.opencontainers.image.title=")
        self.assertLess(runtime_copy, revision_arg)
        self.assertLess(revision_arg, image_label)
